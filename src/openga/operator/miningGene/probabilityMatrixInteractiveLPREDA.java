 ...
 * @version 1.0
 */
public class probabilityMatrixInteractiveLPREDA extends geneStatistics {

    public probabilityMatrixInteractiveLPREDA() {
    }
    int popSize, chromosomeLength;
    //it's an m-by-m array to store the gene results. The i is job, the j is the position(sequence).
    ...
    double e = 0.9; // decide to use prob block or not
    int s = 5; // num of genes to a block
    double Q = 1; // decide how much best individual to the prob
    openga.util.printClass p1 = new openga.util.printClass();

    public void setData(populationI originalPop, populationI duplicatedPopulation, double container[][], double inter[][], double w1, double w2) {
        this.originalPop = originalPop;
        this.duplicatedPopulation = duplicatedPopulation;
        this.popSize = originalPop.getPopulationSize();
        this.chromosomeLength = originalPop.getSingleChromosome(0).genes.length;
        //      container = new double[chromosomeLength][chromosomeLength];
        this.container = container;
        this.inter = container;
...
//                System.out.println();
         ...
         */
        //calculate container by learning rate lamda for the next generation
        for (int i = 0; i < chromosomeLength; i++) {
            for (int j = 0; j < chromosomeLength; j++) {
...
//        System.exit(0);
    ...
    public void assignJobs(double matrix[][], double interp[][]) {
        //To assign jobs onto the chromosome in a population
/*
        System.out.println("container:assign");
        for (int i = 0; i < chromosomeLength; i++) {
            for (int j = 0; j < chromosomeLength; j++) {
                System.out.print(container[i][j] + ",");
            }
        ...
        System.out.println("inter:assign");
        for (int i = 0; i < chromosomeLength; i++) {
            for (int j = 0; j < chromosomeLength; j++) {
                System.out.print(interp[i][j] + ",");
            }
            System.out.println();
        }
*/
        ...
        for (int i = 0; i < numCopied; i++) {  //start numCopied duplicatedPopulation.getPopulationSize()
            ...
            double temporaryInter[][];
            temporaryContainer = dumpArray(container);
            temporaryInter = dumpArray(interp);
            global_index = i;
             ...
             */
            //p1.printMatrix("before: " + i, duplicatedPopulation.getSingleChromosome(i).getSolution());
            generateChromosome(temporaryContainer, temporaryInter, duplicatedPopulation.getSingleChromosome(i));
            //p1.printMatrix("after: " + i, duplicatedPopulation.getSingleChromosome(i).getSolution());
        }
        //System.exit(0);
    }
    ...
    public void generateChromosome(double temporaryContainer[][], double _inter[][], chromosome solution) {
        //strategy
        int prejobIndex = -1;
        int jobIndex = 0;
        int pos = 0;
        double rnd = 0.0;
        int scount = 0;
        int sequence[] = new int[solution.getLength()];
        for (int i = 0; i < solution.getLength(); i++) {
            sequence[i] = solution.genes[i];
        }
        //       for (int t =0;t <chromosomeLength  ; t++ ){
        //       System.out.print(sequence[t] +",");
        //       }
...
//        System.out.println("result:  ");
        for (int j = 0; j < chromosomeLength; j++) {  //
            ...
            if (rnd < e) {
                jobIndex = selectJob_rnd(sequence, temporaryContainer, pos);
            } else {
                if (scount == 0) {
                    scount = s - 1;
                } else {
                ...
                /*
                if( j==0 ){
                jobIndex = (global_index % chromosomeLength); //(int)(chromosomeLength*Math.random());
                }else{
                normArray(temporaryContainer,_inter, pos, prejobIndex);
                jobIndex = selectJob(temporaryContainer, pos); //to get a job by RWS.
                }
                 */
                //first is prob
                normArray(temporaryContainer, _inter, pos, prejobIndex);
                jobIndex = selectJob(sequence, temporaryContainer, pos); //to get a job by RWS.
...
//            p1.printMatrix("\ndrop container", temporaryContainer);
    ...
    public int selectJob_rnd(int _sequence[], double matrix[][], int columnIndex) {    //edit
        int job = 0;   //max_job
        //double AccProb[] = new double[chromosomeLength];
        double prob = 0;  //max_prob
        //double buff = 0.0;
        //double SumFitness = 0.0;

        //for (int i = 0; i < chromosomeLength; i++) {    //To calculate accumulative probability.
        if (matrix[_sequence[columnIndex]][columnIndex] != -1) {
            job = _sequence[columnIndex];
        } else {
            for (int i = 0; i < chromosomeLength; i++) {
                if (matrix[_sequence[i]][columnIndex] != -1) {
                    job = _sequence[i];
                    break;
                }
            }
        ...
        //}
        //System.out.println("same sequence: " + job);
        return job;
    }

    public int selectJob(int _sequence[], double matrix[][], int columnIndex) {    //edit
        int job = 0;
        boolean nullgene = true;
        double AccProb[] = new double[chromosomeLength];
        ...
        for (int j = 0; j < chromosomeLength; j++) {    //To check what is the region it belongs to.
            if ((prob <= AccProb[j]) && (matrix[j][columnIndex] != -1)) {
                job = j;
                nullgene = false;
                break;
            }
        ...
        //System.out.println("prob sequence: " + job);
        return job;
    }
...
//        double prob = 0;  //max_prob
    ...
    //double SumFitness = 0.0;
//        for(int i = 0 ; i < chromosomeLength ; i++){    //To calculate accumulative probability.
//            if((matrix[i][columnIndex] >prob)&&(matrix[i][columnIndex] > 0)){
//                prob = matrix[i][columnIndex];
//                job = i;
//            }
//        }
//        return job;
...
/*
    ...
    public void normArray(double matrix[][], double _inter[][], int columnIndex, int pre_index) {
        ...
        if (pre_index == -1) {
            ...
            double tem1sum = 0, tem2sum = 0;
            for (int i = 0; i < chromosomeLength; i++) {
                if (matrix[i][columnIndex] != -1) {
                    tem1sum += matrix[i][columnIndex];
                    tem2sum += _inter[i][pre_index];
                }
            }

            for (int i = 0; i < chromosomeLength; i++) {
                if (matrix[i][columnIndex] != -1) {
                    matrix[i][columnIndex] /= tem1sum;
                    _inter[i][pre_index] /= tem2sum;
                }
            }
            ...
            for (int i = 0; i < chromosomeLength; i++) {    //To calculate first pos accumulative probability.
                if (matrix[i][columnIndex] != -1) {
                    matrix[i][columnIndex] = (matrix[i][columnIndex] + _inter[i][pre_index]) / 2;
                    SumFitness += matrix[i][columnIndex];
                }
            }
