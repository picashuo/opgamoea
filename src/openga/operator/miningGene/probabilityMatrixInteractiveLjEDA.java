 ...
 * @version 1.0
 */
public class probabilityMatrixInteractiveLjEDA extends geneStatistics {

    public probabilityMatrixInteractiveLjEDA() {
    }
...
/*
        ...
        for (int i = 0; i < numCopied; i++) {  //start numCopied duplicatedPopulation.getPopulationSize()
            ...
            generateChromosome(temporaryContainer, interp, duplicatedPopulation.getSingleChromosome(i));
//            p1.printMatrix(""+i, duplicatedPopulation.getSingleChromosome(i).getSolution());
    ...
    public void generateChromosome(double temporaryContainer[][], double _inter[][], chromosome solution) {
        ...
        int pos = 0;
        double rnd = 0.0;
        double e = 10/(solution.getLength());
...
//        System.out.println("result:  ");
        for (int j = 0; j < chromosomeLength; j++) {  //
            ...
            rnd = Math.random();
            if (j == 0) {
                jobIndex = (int) (chromosomeLength * Math.random()); //(global_index % chromosomeLength) (int)(chromosomeLength*Math.random());
            } else {
                if (rnd > e) {
...
//            p1.printMatrix("\ndrop container", temporaryContainer);
    ...
    public void normArray(double matrix[][], double _inter[][], int columnIndex, int pre_index) {
        ...
        if (pre_index == -1) {
            ...
            for (int i = 0; i < chromosomeLength; i++) {    //To calculate first pos accumulative probability.
                if (matrix[i][columnIndex] != -1) {
                    matrix[i][columnIndex] = matrix[i][columnIndex] * w1 + w2 * _inter[i][pre_index];
                    SumFitness += matrix[i][columnIndex];
                }
            }
